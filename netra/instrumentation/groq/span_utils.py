...
@dont_throw
def set_input_attributes(span: Span, kwargs: Dict[str, Any]) -> None:
...
@dont_throw
def set_model_input_attributes(span: Span, kwargs: Dict[str, Any]) -> None:
    if not span.is_recording():
        return
...
def set_streaming_response_attributes(
    span: Span, accumulated_content: str, finish_reason: Optional[str] = None, usage: Any = None
) -> None:
...
def set_model_streaming_response_attributes(span: Span, usage: Any) -> None:
    if not span.is_recording():
        return
    modified_usage = model_as_dict(usage)
    additional_tokens = modified_usage.get("prompt_tokens_details")
...
def _set_completions(span: Span, choices: Optional[List[Dict[str, Any]]]) -> None:
...
def _dump_content(content: Any) -> str:
    if isinstance(content, str):
        return content
    json_serializable: List[Dict[str, Any]] = []
    for item in content:
