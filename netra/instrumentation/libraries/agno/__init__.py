...
from netra.instrumentation.libraries.agno.version import __version__
...
# The exported OpenTelemetry scope name is a wire contract -- dashboards and
# backend queries key off it -- so it is pinned here rather than derived from
# ``__name__``, which would silently change the moment this file moves.
_TRACER_NAME = "netra.instrumentation.agno"
...
def _resolve_memory_target() -> Optional[Tuple[str, str]]:
    ...
    for module_path, class_name in candidates:
        try:
            ...
            logger.warning(
                "netra.instrumentation.libraries.agno: failed to import %s.%s: %s", module_path, class_name, e
            )
            continue
    return None
...
class NetraAgnoInstrumentor(BaseInstrumentor):  # type: ignore[misc]
    ...
    def _instrument(self, **kwargs: Any) -> Any:
        ...
        try:
            ...
            tracer = get_tracer(_TRACER_NAME, __version__, tracer_provider)
        except Exception as e:
    ...
    def _uninstrument(self, **_kwargs: Any) -> None:
        ...
        try:
            ...
            logger.error("netra.instrumentation.libraries.agno: failed to uninstrument Agent.run/arun: %s", e)
        ...
        try:
            ...
            logger.error(
                "netra.instrumentation.libraries.agno: failed to uninstrument Agent.continue_run/acontinue_run: %s", e
            )
        ...
        try:
            ...
            logger.error(
                "netra.instrumentation.libraries.agno: failed to uninstrument FunctionCall.execute/aexecute: %s", e
            )
        ...
        try:
            ...
            logger.error("netra.instrumentation.libraries.agno: failed to uninstrument Team.run/arun: %s", e)

        try:
            unwrap("agno.workflow.workflow", "Workflow.run")
            unwrap("agno.workflow.workflow", "Workflow.arun")
        except (AttributeError, ModuleNotFoundError) as e:
            logger.error("netra.instrumentation.libraries.agno: failed to uninstrument Workflow.run/arun: %s", e)
        ...
        try:
            ...
            logger.error("netra.instrumentation.libraries.agno: failed to uninstrument Agno models: %s", e)

        try:
            unwrap("agno.vectordb.base", "VectorDb.search")
            unwrap("agno.vectordb.base", "VectorDb.upsert")
        except (AttributeError, ModuleNotFoundError) as e:
            logger.error("netra.instrumentation.libraries.agno: failed to uninstrument VectorDb.search/upsert: %s", e)
        ...
        if self._memory_target:
            ...
            try:
                ...
                logger.error(
                    "netra.instrumentation.libraries.agno: failed to uninstrument %s memory methods: %s", mem_class, e
                )
        ...
        if self._knowledge_target:
            ...
            try:
                ...
                logger.error(
                    "netra.instrumentation.libraries.agno: failed to uninstrument %s.search: %s", know_class, e
                )

        try:
            unwrap("agno.os.app", "AgentOS.get_app")
        except (AttributeError, ModuleNotFoundError) as e:
            logger.error("netra.instrumentation.libraries.agno: failed to uninstrument AgentOS.get_app: %s", e)
