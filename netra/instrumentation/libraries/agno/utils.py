        ...
        for slot in getattr(value, "__slots__", ()):
            if not slot.startswith("_"):
                try:
                    ...
                    logger.debug(
                        "netra.instrumentation.libraries.agno: skipping unset slot %r on %s: %s",
                        slot,
                        type(value).__name__,
                        e,
                    )
...
def serialize_value(data: Any, clean: bool = False) -> Optional[str]:
    ...
    try:
        ...
        logger.debug("netra.instrumentation.libraries.agno: failed to serialize value: %s", e)
...
def update_active_span_with_system_prompt(messages: Any) -> None:
    ...
    try:
        ...
        logger.debug("netra.instrumentation.libraries.agno: failed to update span with system prompt: %s", e)
...
def extract_memory_attributes(instance: Any, args: Tuple[Any, ...], operation: str) -> Dict[str, Any]:
    ...
    if args:
        try:
            ...
            logger.debug("netra.instrumentation.libraries.agno: failed to extract memory input: %s", e)
    ...
    for attr in ("content", "message", "result"):
        ...
        if hasattr(value, "model_dump_json"):
            try:
                ...
                logger.debug("netra.instrumentation.libraries.agno: failed to serialize Pydantic output: %s", e)
        ...
        if not content and str(role) == "assistant":
            ...
            if tool_calls:
                try:
                    ...
                    logger.debug(
                        "netra.instrumentation.libraries.agno: failed to normalize tool_calls in message: %s", e
                    )
...
def set_request_attributes(
    ...
    if extractor:
        try:
            ...
            logger.debug(
                "netra.instrumentation.libraries.agno: failed to extract %s attributes: %s",
                entity_type,
                e,
            )
...
def set_agentos_request_input(
    ...
    try:
        input_data: Dict[str, Any] = {
            "method": scope.get("method", ""),
            "url": build_scope_url(scope),
            "headers": sanitize_asgi_headers(scope.get("headers", [])),
        }
        if body:
            try:
        ...
        logger.debug("netra.instrumentation.libraries.agno: failed to set agentos request input: %s", e)
...
def set_agentos_response_output(
    ...
    try:
        output_data: Dict[str, Any] = {
            ...
            "headers": sanitize_asgi_headers(raw_headers),
        }
        if body:
            try:
        ...
        logger.debug("netra.instrumentation.libraries.agno: failed to set agentos response output: %s", e)
