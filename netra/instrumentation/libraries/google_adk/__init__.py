...
from netra.instrumentation.libraries.google_adk.version import __version__
from netra.instrumentation.libraries.google_adk.wrappers import (
    NoOpTracer,
...
_TRACER_NAME = "netra.instrumentation.google_adk"
...
class NetraGoogleADKInstrumentor(BaseInstrumentor):  # type: ignore[misc]
    ...
    def _instrument(self, **kwargs) -> Any:  # type: ignore[no-untyped-def]
        ...
        try:
            wrap_function_wrapper(
                "google.adk.flows.llm_flows.base_llm_flow",
                "BaseLlmFlow._run_and_handle_error",
                run_and_handle_error_wrapper(tracer),
            )
        except Exception as e:
            logger.error(f"Failed to instrument BaseLlmFlow._run_and_handle_error: {e}")
    ...
    def _uninstrument(self, **kwargs) -> None:  # type: ignore[no-untyped-def]
        ...
        try:
            unwrap("google.adk.flows.llm_flows.base_llm_flow", "BaseLlmFlow._run_and_handle_error")
        except (AttributeError, ModuleNotFoundError):
            logger.debug("Failed to uninstrument BaseLlmFlow._run_and_handle_error")
