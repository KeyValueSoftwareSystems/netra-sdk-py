...
Provides span naming, URL reconstruction, and structured ``input`` / ``output``
attribute serialization mirroring the conventions used by the httpx
instrumentation.  Header sanitization and body parsing are shared with the
other HTTP instrumentations -- see :mod:`netra.instrumentation.http.headers`
and :mod:`netra.instrumentation.http.body`.
"""
...
from netra.instrumentation.http.body import build_response_output
...
def set_span_input(span: Span, scope: Dict[str, Any], body: bytes) -> None:
    ...
    try:
        ...
        span.set_attribute("input", build_response_output(input_data, body))
...
def set_span_output(
    ...
    try:
        output_data: Dict[str, Any] = {
            ...
            "headers": sanitize_asgi_headers(headers),
        }
        span.set_attribute("output", build_response_output(output_data, body))
    except Exception as e:
